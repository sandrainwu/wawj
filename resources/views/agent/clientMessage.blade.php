...
@section('top_title')
<a class="navbar-brand" href="{{ route('agentHome') }}#xiaoxi"><i class="fa fa-chevron-left"></i></a><span class="text-white"><span class="badge badge-danger">{{ $new_count.'/'.$count }}</span> 来自客户的消息</span>
@endsection
                ...
                <tr>
                    <td>
                        <div class="custom-control custom-checkbox">
                              <input type="checkbox" class="custom-control-input" id="customCheck{{ $message->message_id }}">
                              <label class="custom-control-label" for="customCheck{{ $message->message_id }}"></label>
                        </div>
                    </td>
                    <td>
                        <button type="button" class="btn btn-primary btn-sm btn-block" data-toggle="modal" data-target="#exampleModal" data-whatever="{{ $message->subject!=''?$message->subject:$message->message_type }}" value="{{ route('message', ['id' => $message->message_id]) }}">{{ $message->real_name }} <span class="badge badge-info">{{ $message->message_type }}</span></button>
                    </td>
                    <td>
                        <span class="badge badge-secondary">{{ date("Y-m-d H:i",strtotime($message->created_at)) }}</span>
                    </td>
                    <td align="right">
                    @if ($message->replyed==1)
